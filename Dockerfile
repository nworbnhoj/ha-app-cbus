...
# final image.
RUN apk add --no-cache python3 py3-cffi py3-paho-mqtt py3-pip py3-six tzdata && \
    pip3 install --no-cache-dir --break-system-packages serialx-compat
...
ADD . /cbus
WORKDIR /cbus
RUN pip3 install --break-system-packages parameterized setuptools && \
    python3 -m unittest && \
